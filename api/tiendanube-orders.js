...
// El frontend llama a este endpoint pasando el JWT de la sesion de Supabase.
// Este endpoint busca el token de Tienda Nube guardado para ese usuario,
// trae las ordenes de los ultimos 30 dias, y calcula:
//   - bruto: lo que el cliente pago en total (productos + envio)
//   - envio: lo que el cliente pago de envio (se resta para no inflar el numero)
//   - neto:  bruto - envio = la venta real de productos
//
// Mismas variables de entorno que tiendanube-callback.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Solo contamos ordenes pagadas (no canceladas, no pendientes de pago)
    let bruto = 0;
    let envio = 0;

    allOrders.forEach((order) => {
      const paymentStatus = order.payment_status; // 'paid', 'pending', 'voided', etc.
      const orderStatus = order.status; // 'open', 'closed', 'cancelled'
