...
export async function getLaunchWizardDataAction(): Promise<LaunchWizardData> {
  ...
  const [pixelsResult, pagesResult, audiencesResult, existingPostsResult, campaignsResult, scriptsResult, libraryResult] =
    await Promise.all([
  ...
  return { status: 'ready', pixels, pages, audiences, existingPosts, existingCampaigns, scripts, libraryCreatives }
}
