...
### Excepciones documentadas (no vienen de `app.html`)

- **`--divider`** (`rgba(26,26,24,.05)` claro / `rgba(255,255,255,.05)` oscuro): pedido explícito del usuario para las líneas divisorias de la tabla de Campañas — un escalón más sutil que `--border`, sin equivalente en el legado. Usar solo en tablas densas donde `--border` se sienta demasiado marcado; para todo lo demás, `--border`/`--border-2` siguen siendo la fuente de verdad.
...
**Rediseño (post-MVP):** el porteo inicial calcó también el `border-left:2px solid` del legado para marcar el ítem activo — es el patrón `.nav-item.active{border-left-color:...}` de `app.html:246`. En la revisión de diseño posterior (pedida explícitamente por el usuario, "no se siente premium") se reemplazó por un tratamiento sin borde: fondo lleno `bg-accent/[0.16]` + `shadow-[0_2px_14px_-3px_var(--primary-glow)]` (el mismo glow ya usado en `.btn-accent`/`.dash-hero-cta`) + ícono en `text-accent`. Motivo: un borde de acento a la izquierda de un ítem de lista es el patrón más reconocible de "interfaz genérica hecha con IA" — el detector de diseño de este proyecto lo marca como antipatrón (`side-tab`) — y ninguna referencia de SaaS premium (Linear, Vercel, Notion) lo usa para el ítem activo de su nav; todas usan una superficie rellena. El texto inactivo del sidebar también se corrigió de `text-white/65` (gris plano por opacidad) a `#A8ADBB` — el valor real de `.nav-item{color:...}` en `app.html:244`, restaurado porque un gris con tinte azulado es más cómodo de leer sobre el navy `#14161D` que blanco desaturado por opacidad (mismo criterio que ya aplica el resto de la paleta: los grises de texto de esta app siempre están tintados desde un hue, nunca son gris neutro puro). Los captions de sección (`META ADS`, `AJUSTES`, etc.) subieron de `text-white/30` a `text-white/45` en reposo por el mismo motivo — el valor original, calcado de `.sb-section` en `app.html:243`, quedaba por debajo de lo cómodo para texto en mayúsculas de 10-11px.
